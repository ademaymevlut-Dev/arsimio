...
- `academic_years` ve `academic_terms` eklendi. Bu ilk teslimdeki yıl başına tek aktif dönem varsayımı aynı gün kullanıcı kabulüyle yanlış bulunup aşağıdaki yaşam döngüsü düzeltmesiyle kaldırıldı. Dönemlerin yıl sınırında ve çakışmasız olması, çapraz okul FK'si ve izinli durum geçişleri PostgreSQL index/trigger'larıyla korunur. `academics.read/manage` izinleri mevcut `SCHOOL_ADMIN` rollerine bağlandı.
- Bütün okumalar tenant kapsamında; yazmalar permission + Origin/Host kontrolü, Serializable transaction, optimistic revision ve actor/üyelik bağlı append-only audit kullanır. Kalıcı silme yoktur; yeni yılı etkinleştirmek önceki aktif yılı ve bütün dönemlerini audit ile kapatır.
...
## 2026-09-21 — Üç dilli temel ve dönem çeviri modeli

- Desteklenen diller Türkçe (`tr`), Arnavutça (`sq`) ve İngilizce (`en`) olarak tanımlandı. URL'ler değişmeden okul girişi, Okul Admin kabuğu, genel bakış ve öğretim yılı/dönem ekranı sunucu sözlüklerine taşındı; `<html lang>`, tarihler ve Server Action mesajları seçilen dille birlikte değişir.
- Dil tercihi hostname kapsamlı güvenli cookie'de, oturumlu okul kullanıcısı için ayrıca `school_memberships.preferred_locale` alanında tutulur. İki okul hostname'i cookie bakımından birbirine karışmaz; okulun `default_locale` değeri ilk açılış fallback'idir.
- `academic_term_translations` eklendi. Türkçe, Arnavutça ve İngilizce adlar ayrı dönem ID'leri yerine aynı `academic_term_id` altında tutulur; form üç adı zorunlu kılar ve servis üç satırı aynı transaction'da yazar. Tenant bileşik FK'si ve locale/name CHECK constraint'leri migration SQL'indedir.
- `20260921000200_add_i18n_term_translations` bağlı Neon veritabanına expand-only olarak uygulandı; eski `academic_terms.name` deploy uyumluluğu için korunur. Mevcut iki terimin bilinen Türkçe adı taşındı, bilinmeyen çeviriler uydurulmadı. Veritabanı güncel ve bütün mevcut dönemlerin en az bir çeviri satırı var.
- 56/56 birim testi, TypeScript, ESLint, Prisma validate ve production webpack build geçti. Altı gerçek DB kontrolünde üç çevirinin tek dönem kimliğini/tenant'ını paylaşması dahil tüm yazmalar rollback edildi. Agent-browser ile Türkçe → Arnavutça → İngilizce giriş, `html lang`, iki hostname'in bağımsız cookie kapsamı, korumalı route yönlendirmesi ve hata overlay/console kontrolü geçti. Turbopack build ortamın child-process port kısıtına takıldı; Webpack build başarılı. Tarayıcı ve `3000` portu kapatıldı.

## 2026-09-21 — Dönem yaşam döngüsü düzeltmesi

- Kullanıcı kabulünde dönemlerin ayrı ayrı etkinleştirilmesinin okul işleyişine uymadığı netleşti. Aktif öğretim yılının 1. ve 2. dönemleri birlikte kullanılabilir olmalıdır; not, yorum, yoklama ve diğer tarihli kayıtların dönemi kayıt tarihinden otomatik bulunacaktır.
- Yıl başına tek aktif dönem index'i kaldırıldı. Yıl etkinleşince arşivlenmemiş dönemlerin tamamı birlikte etkinleşir; yıl kapanınca tamamı birlikte kapanır. Dönem satırlarındaki elle etkinleştir/kapat CTA'ları kaldırıldı ve doğrudan Server Action çağrıları da reddedilir.
- Okul saat dilimindeki bir zaman damgasını aktif yıl ve tarih aralığına uyan döneme çözen ortak akademik tarih bağlamı eklendi. Önceki tek-dönem akışının aktif yılda kapattığı dönemler düzeltme migration'ı ile yeniden etkinleştirilir; kayıt ve çeviri kimlikleri değişmez.
- `20260921000300_sync_term_lifecycle_with_year` bağlı Neon veritabanına uygulandı. Gerçek kayıtta `2026 / 2027` aktif yılının iki döneminin de `ACTIVE` olduğu ve 21 Eylül ile 1 Mart tarihlerinin iki farklı dönem ID'sine çözüldüğü doğrulandı; 8/8 rollback-only DB kontrolü, 57/57 birim testi, TypeScript, ESLint, Prisma validate ve production Webpack build geçti. Yerel korumalı route doğru giriş ekranına yönlendi; içerik, error overlay ve console kontrolleri geçti. Tarayıcı ve `3000` portu kapatıldı.
- Tarih çözümleyici okulun `timezone` alanını kullanır. İki pilot okul halen `UTC` varsayılanındadır; tarihli ders/not/yoklama modüllerinden önce Okul Ayarları'ndan gerçek IANA saat diliminin seçilmesi açık bir kurulum maddesidir. Bu turda okul konumu tahmin edilerek canlı timezone verisi değiştirilmedi.

## 2026-09-22 — Faz 1 kademe, sınıf, ders ve ders saatleri

- Kullanıcı Faz 1'in okul takvimi/çalışma günlerinden önceki adımlarının tamamlanmasını; takvim başlığında durulup fikir alışverişi yapılmasını istedi. `/academics/structure` Okul Admin çalışma alanı kademe/seviye, sınıf/şube, üç dilli ders kataloğu, yıllık sınıf–ders planı ve ders saatleri sekmeleriyle eklendi. Takvim/çalışma günü tablosu, migration'ı veya UI'si oluşturulmadı.
- Kademe ve seviye ülke/yıl değişkenliğini korumak için ayrıldı. Kademe okul+yıl kapsamında kod, sıra ve üç dilde ad; seviye ise kademe + `PRF/1…12` gibi kod + dil bağımsız sıra taşır. Şubeler seviyeye bağlanıp `4 / 1` biçiminde gösterilir. Sabit ülke enum'u veya zorunlu 1–12 seed'i eklenmedi.
- Ders kataloğu okul genelinde tek ders ID'si + `tr/sq/en` çevirileriyle kuruldu. `course_offerings` bir dersin seçili yılda hangi sınıf/şubede okutulacağını tutar; dönem/öğretmen ataması Faz 4'e bırakıldı. Ders saatleri yıl kapsamlı, üç dilli ve çakışmasızdır.
- `20260922000100_add_academic_structure` bağlı Neon'a uygulandı; uygulama tablo sayısı 28 oldu. İlk denemede PostgreSQL'in 63 karakterlik nesne adı sınırında iki boş indeks adı çakıştı. Yalnız migration'ın oluşturduğu dokuz boş tablo sayımla doğrulanıp temizlendi; indeks adları açık kısa adlara çevrildi ve migration atomik olarak başarıyla yeniden uygulandı. Mevcut okul, kullanıcı, yıl ve dönem verileri değiştirilmedi.
- 63/63 birim testi, TypeScript, ESLint, Prisma format/validate/generate ve production Webpack build geçti. Dokuz gerçek DB kontrolü tenant/yıl kapsamını, tek ID altında çevirileri, tekrar ve saat çakışması engellerini, bağımlı arşivleme/geri alma ve audit'i doğruladı; bütün fixture'lar rollback edildi. Turbopack build ortamın yardımcı port kısıtına takıldı; Webpack build başarılıdır.
- Agent-browser ile doğru okul tenant hostname'inin giriş ekranına yönlendiği ve sayfanın içerik ürettiği doğrulandı. Ayrı browser oturumunda kullanıcı parolası bulunmadığı için korumalı `/academics/structure` içeriğinde tam etkileşimli kabul yapılmadı; servis/DB ve production build doğrulamaları tamamlandı. Geçici browser ve geliştirme sunucusu çalışma sonunda kapatıldı.

## 2026-09-23 — Eski ders kataloğu seed'i ve yeni yıl tekrar kullanım incelemesi

- Eski HorizonEdu ders listesi görselindeki 49 Arnavutça ad aynı yazımla çıkarıldı. Tek ders kimliğine bağlı Türkçe/Arnavutça/İngilizce adları ve `GENERAL` (31), `ELECTIVE` (13), `IGCSE` (5) türleri kodlandı. Seçmeli ve IGCSE dersleri programda ayrı kalırken not modülünde ayrı ortak hanelere bağlanması ürün kararı olarak kaydedildi; not işleme henüz uygulanmadı.
- `20260923000100_add_subject_track` migration'ı bağlı Neon'a uygulandı. Okul Admin ders formu ve listesi ders türünü yönetir. İdempotent seed, önizleme, mevcut kayıt çakışma koruması ve transaction içi `IMPORT` audit'i ile eklendi.
